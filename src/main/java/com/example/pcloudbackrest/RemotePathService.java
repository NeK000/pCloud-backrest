...
final class RemotePathService {
    private RemotePathService() {
    }

    static String normalizeRemoteFolder(String path) {
    ...
    static String join(String folder, String child) {
    ...
    static String parent(String remoteFilePath) {
        String normalized = normalize(remoteFilePath);
        int index = normalized.lastIndexOf('/');
        return index <= 0 ? "/" : normalized.substring(0, index);
    }

    static String filename(String remoteFilePath) {
        String normalized = normalize(remoteFilePath);
        int index = normalized.lastIndexOf('/');
        return index < 0 ? normalized : normalized.substring(index + 1);
    }

    static String relativeRemotePath(Path root, Path file) {
        Path relative = root.toAbsolutePath().normalize().relativize(file.toAbsolutePath().normalize());
        return relative.toString().replace('\\', '/');
    }

    static Path safeResolve(Path root, String relativePath) {
